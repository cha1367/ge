...
COLOR_MAP = {"화이트칼라": "#2563EB", "블루칼라": "#F97316", "기타": "#9CA3AF"}
...
def chart_layout(fig, height: int = 430, showlegend: bool = True):
    fig.update_layout(
        template="plotly_white",
        height=height,
        margin=dict(l=20, r=20, t=55, b=35),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        font=dict(family="Noto Sans KR, Arial", size=13, color="#1F2937"),
...
def fig_manager_bar(manager_df: pd.DataFrame):
    ...
    fig.update_traces(marker_color=["#2563EB", "#60A5FA"], textposition="outside")
...
def fig_wage_compare(wage_group: pd.DataFrame):
    ...
    fig.update_traces(marker_color=["#1D4ED8", "#60A5FA", "#F97316"], textposition="outside")
...
def page_trend(job_share):
    ...
    st.markdown("<div class='insight-card'><b>핵심 해석</b><br>2016~2025년 사이 화이트칼라 비중은 증가한 반면, 블루칼라 비중은 감소했다. 다만 전체 노동시장은 화이트칼라와 블루칼라만으로 구성되지 않으므로 서비스·판매·농림어업 등 기타 직군을 별도로 해석할 필요가 있다.</div>", unsafe_allow_html=True)
...
def page_manager(manager_df):
    ...
    st.markdown(f"<div class='insight-card'><b>핵심 해석</b><br>2025년 기준 관리자를 포함하면 화이트칼라 비중은 {latest.loc['관리자 포함','비중']:.2f}%, 제외하면 {latest.loc['관리자 제외','비중']:.2f}%로 차이는 {diff:.2f}%p다. 따라서 관리자 포함 여부는 수치에는 영향을 주지만, 화이트칼라 비중 증가라는 핵심 추세 자체를 바꾸지는 않는다.</div>", unsafe_allow_html=True)

    st.markdown("<div class='section-title'>관리자 직군 설명</div>", unsafe_allow_html=True)
...
def page_age_edu(age_df, edu_df):
    ...
    st.markdown("<div class='insight-card'><b>핵심 해석</b><br>화이트칼라는 대졸 이상과 30~40대 중심의 비중이 높고, 블루칼라는 고졸 및 50대 이상 비중이 상대적으로 높다. 이는 두 직군이 요구하는 교육 수준과 노동시장 내 연령 구조가 다르다는 점을 보여준다.</div>", unsafe_allow_html=True)
...
def page_region(region_job, region_industry):
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(fig_top_bar(white_top, "행정구역별", "비중", "화이트칼라 비중 상위 지역", "#2563EB"), use_container_width=True)
    with c2:
        st.plotly_chart(fig_top_bar(blue_top, "행정구역별", "비중", "블루칼라 비중 상위 지역", "#F97316"), use_container_width=True)
    st.plotly_chart(fig_top_bar(manu, "시도별", "비중", "제조업 취업자 비중 상위 지역", "#64748B"), use_container_width=True)

    st.markdown("<div class='insight-card'><b>핵심 해석</b><br>지역별 직군 차이는 단순한 직업 선택의 차이라기보다 지역 산업구조와 연결된다. 서울·세종은 사무·전문직 중심 산업 또는 공공행정 기능과 관련해 화이트칼라 비중이 높고, 울산·경남 등 제조업 기반 지역은 블루칼라 비중이 상대적으로 높게 나타난다.</div>", unsafe_allow_html=True)
...
def page_wage(wage_group, latest_job):
    ...
    st.markdown("<div class='insight-card'><b>핵심 해석</b><br>화이트칼라의 임금 수준은 블루칼라보다 높게 나타나지만, 관리자는 임금 수준이 매우 높아 평균값에 큰 영향을 준다. 따라서 임금 분석에서는 관리자 포함 기준과 제외 기준을 함께 제시하는 것이 필요하다.</div>", unsafe_allow_html=True)
...
def page_industry(industry_job):
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(fig_top_bar(white_top, "산업표준", "비중", "화이트칼라 비중이 높은 산업 TOP5", "#2563EB"), use_container_width=True)
    with c2:
    ...
    d["산업표준"] = pd.Categorical(d["산업표준"], major, ordered=True)
    fig = stacked_bar_100(d.sort_values(["산업표준", "직군"]), "산업표준", "직군", "비중", "주요 산업별 직군 구성", x_title="산업")
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("<div class='insight-card'><b>핵심 해석</b><br>정보통신업과 전문·과학·기술 서비스업은 화이트칼라 비중이 높고, 운수·창고업, 건설업, 제조업은 블루칼라 비중이 높다. 따라서 직군 분포는 산업구조와 밀접하게 연결되어 있다고 해석할 수 있다.</div>", unsafe_allow_html=True)
...
def main():
    ...
    st.sidebar.markdown("---")
    st.sidebar.info("CSV 파일은 GitHub 저장소의 data 폴더에 넣고 배포하면 됩니다.")
