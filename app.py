...
def chart_layout(fig, height=420, showlegend=True):
    fig.update_layout(
        ...
        font=dict(family="Noto Sans KR, Arial", size=13, color="#E5E7EB"),
    ...
    fig.update_yaxes(gridcolor="rgba(148,163,184,0.22)", zeroline=False, color="#CBD5E1")
    return fig
...
def page_industry(industry_job):
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(fig_top_bar(white_top, "산업표준", "비중", "화이트칼라 비중이 높은 산업 TOP5", "#38BDF8"), use_container_width=True)
    with c2:
        st.plotly_chart(fig_top_bar(blue_top, "산업표준", "비중", "블루칼라 비중이 높은 산업 TOP5", "#FB923C"), use_container_width=True)

    major = ["제조업", "건설업", "운수 및 창고업", "정보통신업", "금융 및 보험업", "전문, 과학 및 기술 서비스업", "교육 서비스업"]
    d = industry_job[industry_job["산업표준"].isin(major)].copy()

    # Plotly가 pandas Categorical을 숫자 코드처럼 처리하는 경우가 있어
    # 정렬은 보조 컬럼으로 처리하고, 실제 x축은 문자열로 유지한다.
    if d.empty:
        st.markdown(
