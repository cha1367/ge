...
def page_policy() -> None:
    ...
    st.markdown(
        """
<div class="connector" style="border-left-color:#22c55e; background:rgba(34,197,94,0.08); border-color:rgba(34,197,94,0.18);">
    <strong>수정 확인용:</strong> 이 화면이 보이면 정책·결론 파트가 최신 버전입니다.
    핵심 정책은 <strong>우수 중소·중견기업 실무경력 인증 우대전형</strong>입니다.
</div>
""",
        unsafe_allow_html=True,
    )
...
""",
    ...
    with st.expander("교수님 질문 방어용 정리", expanded=True):
        st.markdown(
            """
| 예상 질문 | 답변 |
|---|---|
| 이건 대기업 프리패스 아닌가요? | 아닙니다. 채용을 보장하는 것이 아니라, 우수 중소·중견기업에서 검증된 실무경력을 채용 과정에서 공식적으로 인정하는 협약형 우대전형입니다. |
| 기존 청년정책과 뭐가 다른가요? | 기존 정책은 상담·교육·참여자 수 또는 근속 인센티브 중심인 경우가 많습니다. 이 제안은 중소·중견기업 실무경력을 상위 커리어로 연결하는 사다리 구조에 초점을 둡니다. |
| 중소·중견기업은 인재를 뺏기는 것 아닌가요? | 최소 2~3년 근속 후 인증을 부여하고, 인증 기업에는 인재 유입·교육 지원·평판 효과를 제공하는 방식으로 설계할 수 있습니다. |
| 취업하면 고용보험은 자동으로 가입되는 것 아닌가요? | 맞습니다. 그래서 가입 여부보다 3개월·6개월 이상 피보험 상태가 유지되는지를 성과 기준으로 봐야 합니다. |
| 소비·연금 공백은 실제 손실인가요? | 아닙니다. 잠재 추정치입니다. 따라서 특정 금액을 보전하자는 것이 아니라, 그 공백이 생기는 경로를 줄이는 정책 방향으로 해석해야 합니다. |
"""
        )
