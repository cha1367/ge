...
if page == "프로젝트 개요":
    ...
    with col1:
        ...
        t1 = top_emp.rename(columns={"region_short": "고용률 상위 지역", "regional_employment_rate": "고용률(%)"}).reset_index(drop=True)
        t2 = low_emp.rename(columns={"region_short": "고용률 하위 지역", "regional_employment_rate": "고용률(%)"}).reset_index(drop=True)
        both = pd.concat([t1, t2], axis=1)
        st.dataframe(both, use_container_width=True, hide_index=True)
    with col2:
        section("실업률 보조 확인", "지역별 실업률은 변동성이 클 수 있으므로 고용률을 중심으로 해석하고 실업률은 참고 지표로 사용한다.")
        fig2 = px.bar(
    ...
    callout("최종 정리: <b>본 프로젝트는 청년 취업난을 단순한 고용률 문제가 아니라, 첫 취업 지연·취업 준비 장기화·실업률 밖 청년·불안정 고용·산업별 보상 격차·지역별 고용 여건 차이가 함께 나타나는 구조적 문제로 분석했다.</b>")
